function Configure-IISServer
{
    ...
    try {
        ...
        $RolesandFeatures | Add-WindowsFeature -ComputerName $ServerName -Credential $AdminCreds | Out-Null
        Write-Verbose ('Windows Features {0} Installed on {1}' -f ($RolesandFeatures -join ','),$ServerName)  
    }
    catch {
    ...
    if ($RootDriveLetter -ne 'C')
    {
        ...
        try {
            $Script = {
                ...
                $cert = Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.subject -like "*CN=" + $env:COMPUTERNAME + "*"}
                if ($cert)
                {
